...
public interface ProductRepository extends JpaRepository<Product,Long>, ProductCustomRepository {
    ...
    @Query("SELECT p FROM Product p WHERE p.status=:status AND p.member.userKey=:userKey AND p.isUse=true")
    Page<Product> findByStatusAndUserKey(@Param("status") ProductStatus status, @Param("userKey") String userKey, Pageable pageable);
